...
def home(request):
    # posts = Post.objects.all() # 이렇게 부르면 모조리
...
def new_comment(request, post_id):
    ...
    if filled_form.is_valid():
        finished_form = filled_form.save(commit=False) # 아직은 저장하지 말고
        finished_form.post = get_object_or_404(Post, pk=post_id) # 어느 글의 댓글인지 담고
